...
int64_t maximum_chain(std::span<const char> snapshots) {
  ...
  for (auto &&[idx, shape] :
       shapes | std::views::enumerate | std::views::drop(1)) {
    // This brick can be removed without causing any bricks to fall
    if (can_remove(idx))
      continue;

    // Keep track of bricks that are dropping
    std::vector<bool> is_dropping(std::ssize(shapes), false);
    std::queue<int64_t> q;

    // We make this brick drop
    is_dropping[idx] = true;
    q.push(idx);
    ...
    while (not q.empty()) {
      ...
      for (int64_t supported : shapes[idx].supports) {
        // If all bricks that support this brick are dropping
        if (std::ranges::all_of(shapes[supported].supported_by,
                                [&](int64_t i) { return is_dropping[i]; })) {
          q.push(supported);
          is_dropping[supported] = true;
        }
      }
    }
    // How many bricks (other than the first one) did drop?
    sum += std::ranges::count_if(is_dropping, std::identity{}) - 1;
  }
