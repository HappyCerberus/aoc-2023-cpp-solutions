...
int64_t sum_of_distances_old_universe(std::string_view map) {
  ...
  auto distance = [&](Coord first, Coord second) {
    ...
    return std::abs(first.col - second.col) + (1000000 - 1) * columns +
           std::abs(first.row - second.row) + (1000000 - 1) * rows;
  };
  ...
  auto all_galaxies = map | std::views::split('\n') | std::views::enumerate |
                      std::views::transform([](auto &&e) {
                        ...
                        return line | std::views::enumerate |
                               ...
                               std::views::transform([idx](const auto &f) {
                                 return Coord{idx, std::get<0>(f)};
                               });
                      }) |
                      std::views::join;
  // Fold over the galaxies, for each producing the sum of distances to all
